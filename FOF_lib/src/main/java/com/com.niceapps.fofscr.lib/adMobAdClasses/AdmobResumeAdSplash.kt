...
class AdmobResumeAdSplash(activity: Activity?=null, val adId: String, onAdDismissed: (() -> Unit)? = null, onAdFailed: (() -> Unit)? = null, onAdTimeout: (() -> Unit)? = null, onAdShowed: (() -> Unit)? = null) {
    ...
    private val timeoutRunnable = Runnable {
        if (appOpenAd == null) {
            onAdTimeout?.invoke()
            dismissWaitDialog()
            Log.i("NICE_APPS_ADS_TAG", "Admob: Resume : Timeout()")
            currentActivity?.let {
    ...
    private fun fetchAd(onAdDismissed: (() -> Unit)? = null, onAdFailed: (() -> Unit)? = null, onAdShowed: (() -> Unit)? = null) {
        ...
        val loadCallback: AppOpenAd.AppOpenAdLoadCallback = object : AppOpenAd.AppOpenAdLoadCallback() {
            override fun onAdLoaded(ad: AppOpenAd) {
                Log.i("NICE_APPS_ADS_TAG","Admob: Resume : onAdLoaded()")
                appOpenAd = ad
                if (currentActivity?.localClassName != null || currentActivity?.localClassName.equals("")) {
            ...
            override fun onAdFailedToLoad(p0: LoadAdError) {
                ...
                Log.i("NICE_APPS_ADS_TAG", "Admob: Resume : onAdFailedToLoad()")
                currentActivity.let {
                    if (BuildConfig.DEBUG) {
        ...
        if (!isShowingAd && isAdAvailable()) {
            fullScreenContentCallback = object : FullScreenContentCallback() {
                override fun onAdDismissedFullScreenContent() {
                    Log.i("NICE_APPS_ADS_TAG","Admob: Resume : onAdDismissedFullScreenContent()")
                    isShowDialog = false
                    dismissWaitDialog()
                    timeoutHandler.removeCallbacks(timeoutRunnable)
                    onAdDismissed.let {
                ...
                override fun onAdFailedToShowFullScreenContent(p0: AdError) {
                    Log.i("NICE_APPS_ADS_TAG","Admob: Resume : onAdFailedToShowFullScreenContent()")
                    isShowDialog = false
                    dismissWaitDialog()
                    timeoutHandler.removeCallbacks(timeoutRunnable)
                    onAdFailed.let {
                ...
                override fun onAdShowedFullScreenContent() {
                    Log.i("NICE_APPS_ADS_TAG","Admob: Resume : onAdShowedFullScreenContent()")
                    isShowingAd = true
                    isShowDialog = false
                    dismissWaitDialog()
                    timeoutHandler.removeCallbacks(timeoutRunnable)
    ...
    private fun showWaitDialog() {
        Log.i("NICE_APPS_ADS_TAG", "Admob: Resume : showWaitDialog()")
        if (isShowingDialog) {
            currentActivity?.let {
    ...
    private fun dismissWaitDialog() {
        Log.i("NICE_APPS_ADS_TAG", "Admob: Resume : dismissWaitDialog()")
        currentActivity?.let {
            if (!(currentActivity as Activity).isFinishing) {
                AdLoadingDialog.dismissDialog(currentActivity!!)
            }
